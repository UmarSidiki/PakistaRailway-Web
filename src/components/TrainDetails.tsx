...
// Define theme constants for better maintainability
const THEME = {
  colors: {
    emerald: {
      light: "#e7f6ef",
  ...
  shadows: {
    card: "shadow-[0_22px_40px_-36px_rgba(95,75,60,0.45)]",
    cardHover: "hover:shadow-[0_28px_55px_-32px_rgba(95,75,60,0.5)]",
    inset: "shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]",
  },
};
...
interface StopStateProps {
  state: "passed" | "current" | "upcoming" | "normal";
  children: ReactNode;
}
...
// Define styles for different stop states
...
/**
 * Calculates the dwell time between arrival and departure
 * @param arrival - Arrival time in HH:MM format
 * @param departure - Departure time in HH:MM format
 * @returns Dwell time in minutes or null if invalid
 */
...
/**
 * Determines the state of a train stop based on live position
 * @param train - Train with route information
 * @param stop - Train stop to evaluate
 * @param index - Index of the stop in the route
 * @returns State of the stop
 */
const getStopState = (
  train: TrainWithRoute,
  stop: TrainStop,
...
/**
 * Custom hook to calculate upcoming stop information
 * @param train - Train with route information
 * @returns Object with distance and arrival estimate
 */
const useUpcomingStopInfo = (train: TrainWithRoute) => {
  ...
  return { upcomingDistanceKm, upcomingArrivalEstimate };
};

// --- Sub-components for a Cleaner Structure ---

// Component to style stops based on their state
const StopStateWrapper: FC<StopStateProps> = ({ state, children }) => {
  const getStyles = () => {
    switch (state) {
      case "passed":
        return "border-[#e4d7c5] bg-[#f4e6d6] text-[color:var(--ink-muted)]/85 opacity-80";
      case "current":
        return "border-[#2c7f68] bg-gradient-to-br from-[#e7f6ef] to-[#fdfbf8] text-[#2c7f68] shadow-[0_20px_40px_-34px_rgba(44,127,104,0.55)] ring-2 ring-[#b9e0d0]";
      case "upcoming":
        return "border-[#cde5db] bg-[#f4fbf7] text-[#2c7f68]";
      default:
        return "border-[color:var(--stroke)] bg-[#fffaf3] text-[color:var(--ink-muted)] hover:border-[#d8c8b4]";
    }
  };

  return (
    <div className={`${getStyles()} ${state === "current" ? "transform scale-105" : ""}`}>
      {children}
    </div>
  );
};
...
const InfoCard: FC<InfoCardProps> = ({
  ...
  return (
    <div
      className={`group rounded-2xl border ${colors.shell} p-4 shadow-[0_22px_40px_-36px_rgba(95,75,60,0.45)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_28px_55px_-32px_rgba(95,75,60,0.5)] lg:p-5 h-full flex flex-col`}
    >
      ...
      <div className={`mt-4 text-sm leading-relaxed ${colors.text} flex-grow`}>
        {children}
      </div>
    </div>
  );
...
// Component to display train header information
const TrainHeader: FC<{ train: TrainWithRoute }> = ({ train }) => {
  const live = train.livePosition;
...
// Component to display upcoming stop information
const UpcomingStopCard: FC<{ train: TrainWithRoute }> = ({ train }) => {
  const { upcomingDistanceKm, upcomingArrivalEstimate } = useUpcomingStopInfo(train);
  const live = train.livePosition;

  if (!train.upcomingStop) {
    return (
...
// Component to display live telemetry
const LiveTelemetryCard: FC<{ train: TrainWithRoute }> = ({ train }) => {
  const live = train.livePosition;
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      ...
      <div className="rounded-xl bg-emerald-50/70 p-3 text-xs text-emerald-800 shadow-inner flex-grow">
...
// Component to display a single stop in the route
const RouteStop: FC<{
  train: TrainWithRoute;
  stop: TrainStop;
  index: number;
  isLast: boolean;
  stopRefs: React.MutableRefObject<Map<string, HTMLDivElement>>;
}> = ({ train, stop, index, isLast, stopRefs }) => {
  ...
  let expectedArrival: ReturnType<typeof calculateExpectedArrival> = null;
...
// Component to display the route schedule timeline
const RouteSchedule: FC<{ train: TrainWithRoute }> = ({ train }) => {
        ...
        >
          <div className="flex gap-2 lg:gap-3 min-w-max px-2 lg:px-6">
            {train.route.map((stop, index) => (
              <RouteStop
                key={`${train.TrainId}-${stop.StationId}-${stop.OrderNumber}`}
                train={train}
                stop={stop}
                index={index}
  ...
  return (
    <div className="flex flex-col gap-4 rounded-3xl border border-[color:var(--stroke)] bg-gradient-to-br from-[#fff9f1] via-[#fff4e4] to-[#fffdf8] p-4 shadow-[0_28px_60px_-38px_rgba(95,75,60,0.55)] backdrop-blur lg:gap-6 lg:p-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:gap-5">
        ...
        >
          <UpcomingStopCard train={train} />
        </InfoCard>

        <InfoCard
          title="Live Telemetry"
          subtitle={train.livePosition ? "Realtime sensors" : "Awaiting signal"}
          accent="emerald"
          icon={
            <svg
              className="h-5 w-5"
        ...
        >
          <LiveTelemetryCard train={train} />
        </InfoCard>
      </section>

      {/* Route Schedule Timeline */}
      <RouteSchedule train={train} />
    </div>
  );
};